...
"""放置全部資料集的資料夾"""
data_dir_path = './data/examples/flower_classification'


target_label_file_name = 'mapping.csv'
...
def load_data(Gray2RGB=False, mean_proc=False, test_size=0.25, img_size=img_size):
    ...
    target_labels = dict()
    with open(target_label_file_path, newline='') as csvfile:
        all_lines_csv = csv.reader(csvfile, delimiter=',')
        next(all_lines_csv, None)
        for item in all_lines_csv:
            target_labels[item[0]] = item[1]
...
def load_one_test_data(pic,Gray2RGB=False, mean_proc=False, img_size=img_size):
    img_path_list = []
    img_path_list.append(pic)
    X_test = []
    X_id = []
    for path in img_path_list:
        img = cv2.imread(path, 0)
