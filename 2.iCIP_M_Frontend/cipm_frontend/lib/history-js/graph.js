...
//var localhostURL="127.0.0.1:56090";
var localhostURL ="localhost:56090"
/* function buttonaggre(){
    ...
    if (lock) {
        ...
        var successCallback = function (data) {
            ...
            for (var item in data) {
                ...
                result.yaxis = 1; // Flot
                result.yAxis = 1 - 1; // Highcharts

                dataArray.push(result);
                dataPointCount += data[item].length;
            }
            ...
            if (isHighChartsLoaded())
                showHighChartsChart("(Click and drag to zoom)", [{}], dataArray);
            else
                showFlotChart("(Click and drag to zoom)", [{}], dataArray);
            $status.html("");
...
function buildKairosDBQuery() {
	...
	var startTimeval=$("#startTime").val();
	var startTimeAbsolute = new Date(startTimeval);
	...
	/* alert(startTimeRelativeValue);
	alert($("#startRelativeUnit").val()); */
	
	var endTimeval=$("#endTime").val();
	var endTimeAbsolute = new Date( $("#endTime").val());
	/* alert(endTimeval); */
	/* var endTimeRelativeValue = $("#endRelativeValue").val(); */
	...
	if (endTimeval != '') {
		...
		/* alert(endRelativeValue);
		alert($("#endRelativeUnit").val()); */
		if (endRelativeValue) {
...
{
    $.get('http://'+localhostURL + '/api/postgressql').then(function (results) {
        metricNames = [];
        var temp = [];
        ...
        resultArray.forEach(function (ele) {
            ...
            if(ele!=""){
				...
				temp = temp.concat(ele.split("=")[1].split(","));
			}    
        });  
        metricNames = temp;
    });
